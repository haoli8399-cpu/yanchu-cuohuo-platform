...
> 版本：v2.2
> 创建时间：2026-07-01
> 最后更新：2026-07-02（v2.2：新增 Agent 自验证闭环 + 输出格式强制校验 + 独立审查Agent + 文件写隔离）
> 状态：✅ 已确认，正式执行
...
| Agent | 职责范围 | 技术栈 | 产物 |
|-------|---------|--------|------|
| **Orchestrator**（本窗口） | 契约编写 → 任务拆解 → 派发 → 格式校验 → 审查 → 构建验证 → 部署验证 → 文档同步 | — | API_CONTRACT.md / 部署 / 文档 |
...
| **Agent-Reviewer** ★ 新增 | 独立代码审查（不参与开发，避免"自己审自己"盲区） | — | 审查报告 |
...
| 8 | **Agent 自验证闭环** | 后端 Agent 必须 curl 验证自己写的 API；前端 Agent 必须 npm run build 通过；附输出日志 |
| 9 | **输出格式强制校验** | OC 收到 Agent 返回后，先校验格式完整性，缺字段直接打回 |
...
─────────────────────────────────────
  ...
  子 Agent 开发（含自验证闭环 ★）
    ...
    ├── 写代码
    ├── ★ 自验证：后端 curl 测试关键 API / 前端 npm run build
    ├── ★ 自验证失败 → 修复 → 再验证 → 直到通过
    ├── 遇到阻塞 → 标记 blocker（含阻塞原因 + 需要谁决策）
    └── 返回标准结果（代码路径 + 变更摘要 + 自测 + ★ 附构建/验证日志）
    │
    ▼
  ┌─────────────────────────────────────────────┐
  │  OC 格式校验（审查前置 ★ 新增）                  │
  │                                               │
  │  ★ 检查 Agent 返回是否包含所有必填字段：          │
  │    [ ] code_paths（文件列表不为空）              │
  │    [ ] self_test.passed（必须为 true）          │
  │    [ ] self_test.build_output（前端/后端必填）   │
...
| **★ 文件写隔离** | Agent 只能修改指定目录（backend/ / frontend-admin/ / frontend-agent/），禁止跨目录写 |
| **★ 审查 Agent 独立** | 审查任务通过 delegate_task 派发给 Agent-Reviewer，OC 只看审查结论 |
...
| R7 | **Agent 返回格式不完整** | OC 无法判断任务是否真正完成 | 3次 | ★ 格式校验前置，缺字段直接打回 |
| R8 | **OC 同时做审查+开发** | "自己审自己"盲区，漏掉内容错误 | 1次 | ★ 独立审查 Agent |
...
>   v2.1 (2026-07-02) — 新增构建验证 + 部署冒烟测试 + 内容抽查 + 单次≤3任务 + 风险登记册
>   v2.0 (2026-07-01) — Contract-First + 分层审查 + Peer Review + 集成测试
