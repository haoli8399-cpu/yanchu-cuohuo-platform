...
const PRESET_TEMPLATES: AdjustmentTemplate[] = [
  {
    ...
    category: 'lineup', usage_count: 23, created_at: '2026-06-15T10:00:00Z',
  },
  {
    id: 'tpl-2',
    name: '升级为领衔演员',
    ...
    category: 'lineup', usage_count: 15, created_at: '2026-06-16T14:30:00Z',
  },
  {
    id: 'tpl-3',
    name: '周末档期加价',
    ...
    category: 'price', usage_count: 18, created_at: '2026-06-17T09:15:00Z',
  },
  {
    id: 'tpl-4',
    name: '远程附加费',
    ...
    category: 'price', usage_count: 12, created_at: '2026-06-18T11:00:00Z',
  },
  {
    id: 'tpl-5',
    name: '增加定制段子',
    ...
    category: 'general', usage_count: 8, created_at: '2026-06-19T16:45:00Z',
  },
  {
    id: 'tpl-6',
    name: '跳过合同模式',
    ...
    category: 'contract', usage_count: 31, created_at: '2026-06-20T08:30:00Z',
  },
  {
    id: 'tpl-7',
    name: '上传PDF合同',
    ...
    category: 'contract', usage_count: 20, created_at: '2026-06-21T13:00:00Z',
  },
  {
    id: 'tpl-8',
    name: '增加赞助权益',
    ...
    content: '【内容调整】\n增加品牌赞助权益：\n{{权益列表}}\n品牌方需自行制作物料（话筒贴/展架等）。',
    category: 'general', usage_count: 6, created_at: '2026-06-22T10:20:00Z',
  },
];
...
const AdjustmentTemplates: React.FC<AdjustmentTemplatesProps> = ({
  ...
  const [templates, setTemplates] = useState<AdjustmentTemplate[]>(PRESET_TEMPLATES);
  const [showAddModal, setShowAddModal] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState('');
  const [filterCategory, setFilterCategory] = useState<AdjustmentTemplate['category'] | 'all'>('all');
  ...
  const handleAddTemplate = (data: Omit<AdjustmentTemplate, 'id' | 'usage_count' | 'created_at'>) => {
  ...
  const filtered = templates.filter((t) => {
    ...
    const matchKeyword = !searchKeyword || t.name.includes(searchKeyword) || t.content.includes(searchKeyword);
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        />
        <Button type="primary" icon={<PlusOutlined />} style={{ minHeight: 44 }} onClick={() => setShowAddModal(true)}>
          新建模板
        </Button>
      </Space>
      ...
      {filtered.length === 0 ? (
        <Empty
          ...
          description={searchKeyword ? '未找到匹配的模板' : '暂无调整模板，点击「新建模板」创建'}
        >
          {!searchKeyword && (
                ...
                <Card
                  ...
                  actions={[
                    <Tooltip title="复制内容" key="copy">
                      <Button type="text" icon={<CopyOutlined />} style={{ minHeight: 44, minWidth: 44 }}
                        onClick={(e) => { e.stopPropagation(); handleCopy(tpl.content); }} />
                    </Tooltip>,
                    onSelect && (
                    ...
                    <Popconfirm key="delete" title="确定删除此模板？"
                      onConfirm={(e) => { e?.stopPropagation(); handleDelete(tpl.id); }}
                      okText="确定" cancelText="取消">
                      <Tooltip title="删除模板">
                        <Button type="text" danger icon={<DeleteOutlined />}
                          style={{ minHeight: 44, minWidth: 44 }} onClick={(e) => e.stopPropagation()} />
                      </Tooltip>
                    </Popconfirm>,
                  ].filter(Boolean)}
                >
                    ...
                    description={
                      <div>
                        <Paragraph ellipsis={{ rows: 3, expandable: false }}
                          style={{ marginBottom: 8, whiteSpace: 'pre-wrap', fontSize: 13, color: '#666',
                            background: '#fafafa', padding: 8, borderRadius: 4 }}>
                          {tpl.content}
                        </Paragraph>
                        <Text type="secondary" style={{ fontSize: 12 }}>已使用 {tpl.usage_count} 次</Text>
                      </div>
                    }
                  />
                </Card>
